...
from mybot.common.charm import get_charm
from mybot.common.interaction_stats import get_auto_reply_trigger_count
from mybot.common.json_store import load_json, mutate_json
from mybot.common.streak import get_current_streak
...
SLOT_STATS_FILE = BOT_ROOT / "data" / "slot_stats.json"
...
ACHIEVEMENTS = (
    ...
    TitleAchievement("secret_slot_grape", "青藤摘星手", "秘闻", "青藤摘星", "slot_grape_or_better_count", 1, "图案转盘转出过葡萄三连或更高奖面", 9100, True),
)
...
def _slot_easter_egg_metrics(user_id: str, group_id: str) -> dict[str, int]:
    data = load_json(SLOT_STATS_FILE, {"users": {}})
    record = data.get("users", {}).get(_title_key(group_id, user_id), {})
    if not isinstance(record, dict):
        return {"slot_grape_or_better_count": 0}
    return {
        "slot_grape_or_better_count": _safe_int(record.get("grape_or_better_count", 0)),
    }
...
def collect_title_metrics(user_id: str, group_id: str, points_info: dict | None = None) -> dict[str, int]:
    ...
    metrics.update(_slot_easter_egg_metrics(user_id, group_id))
...
def record_slot_grape_or_better(user_id: str, group_id: str) -> int:
    ...
    def mutator(data: dict) -> int:
        ...
        record = users.setdefault(
            key,
            {
                "user_id": user_id,
                "group_id": group_id,
                "grape_or_better_count": 0,
            },
        )
        record["user_id"] = user_id
        record["group_id"] = group_id
        record["grape_or_better_count"] = _safe_int(record.get("grape_or_better_count", 0)) + 1
        return int(record["grape_or_better_count"])

    return mutate_json(SLOT_STATS_FILE, {"users": {}}, mutator)
...
def get_new_title_achievements(status: dict, hidden_only: bool | None = None) -> list[TitleAchievement]:
    achievements = [
        TITLE_BY_ID[title_id]
        for title_id in status.get("new_ids", [])
        if title_id in TITLE_BY_ID
    ]
    if hidden_only is None:
        return achievements
    return [achievement for achievement in achievements if achievement.hidden is hidden_only]
...
def render_title_list(user_id: str, group_id: str, points_info: dict | None = None) -> str:
    ...
    for achievement in ALL_TITLES:
        ...
        lines.append(f"{'✓' if unlocked else '·'} {achievement.title}")

    lines.append("『名号先看有无，细则改日慢翻。』")
    return "\n".join(lines)


def resolve_title(identifier: str, unlocked_ids: set[str] | None = None) -> TitleAchievement | None:
    text = identifier.strip().lower()
    if not text:
        return None

    for achievement in ALL_TITLES:
        if unlocked_ids is not None and achievement.hidden and achievement.id not in unlocked_ids:
            continue
        candidates = {achievement.id.lower(), achievement.title.lower(), achievement.name.lower()}
...
def render_title_detail(identifier: str, user_id: str, group_id: str, points_info: dict | None = None) -> str | None:
    status = sync_user_titles(user_id, group_id, points_info)
    unlocked_ids = set(status["record"].get("unlocked", ["default"]))
    achievement = resolve_title(identifier, unlocked_ids)
    if not achievement:
        return None
    ...
    if achievement.id == "default":
        progress_text = "初见本猫便已入册"
    elif unlocked:
        progress_text = "已入册"
    else:
        current = metrics.get(achievement.metric, 0)
        progress_text = f"当前进度：{min(current, achievement.threshold)}/{achievement.threshold}"

    return "\n".join(
        [
            "🎖 李太白给·称号说明 🎖",
            "=" * 22,
            "（指尖点过册页）这一笔来历，在此处写得分明。",
            "=" * 22,
            f"称号：{achievement.title}",
            f"名册状态：{'已得' if unlocked else '未得'}",
            f"来历：{achievement.description}",
            progress_text,
            "『名号各有来路，翻到此页便明。』",
        ]
    )
