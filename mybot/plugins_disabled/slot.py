...
from mybot.common.titles import get_new_title_achievements, record_slot_grape_or_better, sync_user_titles
...
GRAPE_OR_BETTER_PAYOUT = PAYOUTS["🍇"]
...
async def handle_slot(bot: Bot, event: MessageEvent, args: Message = CommandArg()):
    ...
    if win_type == "triple":
        ...
        if PAYOUTS.get(win_symbol, 0) >= GRAPE_OR_BETTER_PAYOUT:
            record_slot_grape_or_better(user_id, group_id)
            title_status = sync_user_titles(user_id, group_id, get_points(user_id, group_id))
            hidden_titles = [achievement.title for achievement in get_new_title_achievements(title_status, hidden_only=True)]
            if hidden_titles:
                hidden_title_text = f"\n🎖 天外名号：{'、'.join(hidden_titles)} 已入册"
    ...
    hidden_title_block = f"{hidden_title_text}\n" if hidden_title_text else ""
