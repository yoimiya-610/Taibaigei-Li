...
from common.ai_client import chat_completion, is_configured, prompt_completion
from common.charm import change_charm
from common.command_registry import is_known_command
from common.config import get_random_reply_chance
from common.feature_flags import is_feature_enabled
from common.help_registry import HelpItem
from common.interaction_stats import add_auto_reply_trigger
...
async def handle_ai_chat(event: MessageEvent):
    if not is_feature_enabled("ai_chat"):
        await ai_chat.finish("（合上折扇）普通AI对话暂时歇业中。")
    ...
    if not is_configured():
        await ai_chat.send("未配置 `DEEPSEEK_API_KEY`，当前无法使用 AI 对话。")
        return
    
    chat_history[user_id].append({"role": "user", "content": msg})
    ...
    try:
        ...
        await ai_chat.send(f"（扶额）本猫的脑子好像宕机了...\n稍后再试试吧~")
